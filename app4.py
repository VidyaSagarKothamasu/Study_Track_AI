...
with left:
    ...
    with st.form("log_form"):
        ...
        if submit:
            new_row = pd.DataFrame({
                # ✅ FIXED (consistent datetime format)
    ...
    st.markdown('<div class="card-container animate-fade-in"><h3>📈 Performance Trends</h3>', unsafe_allow_html=True)
...
with right:
    ...
    # 📂 DATA + MODEL
    st.markdown('<div class="card-container animate-fade-in"><h3>📂 Data & Model Management</h3>', unsafe_allow_html=True)
    ...
    # ================= QUICK RETRAIN =================
    if r1.button("Quick Retrain"):
        try:
            latest_df = load_dataset()

            if len(latest_df) < 2:
                st.warning("⚠️ Not enough data to retrain")
            else:
                model, scaler, X, X_scaled = train_model(latest_df)
                ...
                st.success(f"⚡ Quick Retrain Done on {len(latest_df)} records!")
    ...
    # ================= FULL RETRAIN =================
    if r2.button("Full Rebuild", use_container_width=True):
        ...
        if len(logs) > 0:
            try:
                ...
                st.write("New Logs Added:", len(logs_copy))
                st.write("Total Dataset:", len(updated_df))
